    ...
    <section id="contato" className="relative z-10 py-16 lg:py-20 overflow-hidden bg-[#f1f5f9] text-left">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          {/* COLUNA DIREITA */}
          <motion.div
            className="bg-bg-alt border border-card-border rounded-card p-8 shadow-sm"
            initial="hidden"
            whileInView="visible"
            viewport={viewport}
            ...
            {status === "success" ? (
              ...
              <form action={handleSubmit} className="flex flex-col gap-5">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input id="nome" name="nome" type="text" required aria-required="true" placeholder="João Silva" className="w-full px-4 py-3 rounded-xl border border-card-border bg-white text-text text-sm placeholder:text-text-muted/50 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-text" htmlFor="empresa">Empresa</label>
                  <input id="empresa" name="empresa" type="text" required aria-required="true" placeholder="Empresa Ltda." className="w-full px-4 py-3 rounded-xl border border-card-border bg-white text-text text-sm placeholder:text-text-muted/50 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-text" htmlFor="email">E-mail corporativo</label>
                  <input id="email" name="email" type="email" required aria-required="true" placeholder="joao@empresa.com.br" className="w-full px-4 py-3 rounded-xl border border-card-border bg-white text-text text-sm placeholder:text-text-muted/50 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-text" htmlFor="desafio">Qual é seu maior desafio hoje?</label>
                  <textarea id="desafio" name="desafio" rows={3} required aria-required="true" placeholder="Ex: Nosso processo de faturamento é 100% manual e toma 2 dias por mês..." className="w-full px-4 py-3 rounded-xl border border-card-border bg-white text-text text-sm placeholder:text-text-muted/50 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all resize-none" />
                </div>
                <div className="mt-2 pt-2">
                  ...
                  {status === "error" && <p className="text-center text-xs text-red-500 mt-3">Erro ao enviar. Tente novamente.</p>}
                  {status === "idle" && <p className="text-center text-xs text-text-muted mt-3">Sem compromisso. Sem spam. Resposta em até 24h.</p>}
                </div>
              </form>
            )}
          </motion.div>
